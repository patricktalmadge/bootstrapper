...
class BootstrapperServiceProviderSpec extends ObjectBehavior
{
    ...
    function it_can_register_things()
    {
        ...
        $classes = [
            ...
            'bootstrapper::config' => 'Bootstrapper\\Bridges\\Config\\Laravel4Config',
        ...
        foreach ($classes as $class => $className) {
            if (!$this->app->make($class) instanceof $className) {
                throw new Exception(
                    "Expected an instance of $className when resolving $class. Instead recieved: " . get_class(
                        $this->app->make($class)
                    )
                );
            }
    ...
    function it_falls_over_on_laravel_5()
    {
        if (method_exists(
            new BootstrapperServiceProvider($this->app),
            'package'
...
class Laravel4FakeApp implements \ArrayAccess
{
