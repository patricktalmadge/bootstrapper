...
class ButtonSpec extends ObjectBehavior
{
    ...
    {
        $this->withValue('foo')->render()->shouldBe(
            "<button type='button' class='btn btn-default'>foo</button>"
        );
    }
    ...
    function it_can_append_an_icon()
    {
        ...
        $icon->shouldReceive('bar')->andReturn(
    ...
    function it_allows_you_to_use_a_shortcut_method()
    {
        ...
        foreach ($types as $type) {
            ...
            $this->$type($type)->render()->shouldBe(
                "<button type='button' class='btn btn-{$class}'>{$type}</button>"
            );
        }
    }
    ...
    {
        $this->asLinkTo('foo')->render()->shouldBe(
            "<a class='btn btn-default' href='foo'></a>"
        );
    }
