...
class MediaObjectTest extends BootstrapperWrapper
{
    ...
    {
        if (!$image) {
            $image = '<img src="http://test/bar" class="media-object" alt="bar">';
        }
        if (!$pull) {
            $pull = 'left';
        }
    ...
    public function testWithImage()
    {
        ...
        $matcher = $this->createMatcher(
            '<img src="http://test/bar" class="foo media-object" data-foo="bar" alt="alt">'
        );
    ...
    public function testMagicTitles($title, $expected)
    {
        ...
        $media = MediaObject::create('foo', 'bar', $this->testAttributes)->$title(
            'foobar',
            $this->testAttributes
        )->__toString();
